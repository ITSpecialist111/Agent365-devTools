...
### Added
- `query-entra blueprints` (alias: `query-entra list-blueprints`) - lists Agent Identity Blueprints in the tenant so operators can identify orphaned dev/test blueprints when Frontier AI Teammate slots appear exhausted.
...
### Changed
- `setup blueprint` and `setup permissions` help text now calls out that granular setup commands do not register the MCP Platform messaging endpoint. For M365 agents, use `setup all --m365` or run `setup blueprint --endpoint-only --m365` after granular setup.
...
### Fixed
- `a365 publish` now fails fast when `agentBlueprintDisplayName` is longer than 30 characters because that value is written to `manifest.json` `name.short`, which Microsoft 365 Admin Center rejects.
